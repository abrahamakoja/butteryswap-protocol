  ...
  contract BorrowRequestFactory {
      ...
      error BorrowRequestFactory__TransferFailed(address feeAddress, uint256 feeAmount);
      ...
      constructor(address supportedTokensAddress) {
       ...
       receive() external payable {}//delete later
      ...
      function _rawCreateBorrowRequest(
          ...
          if (!success)
              revert BorrowRequestFactory__TransferFailed(
                  address(this),
                 calculateCollateralAmountMinusFee(collateralAmount, i_originationFee)
              );
  
      }
  
       // move to the admin
       function calculateOriginationFee(uint256 collateralAmount,uint256 originationFee) private pure returns(uint256){
         return ((collateralAmount*originationFee)/100) * 1 ether;
      }
      function calculateCollateralAmountMinusFee(
          uint256 collateralAmount,
          uint256 originationFee
      ) private pure returns (uint256) {
          return (collateralAmount - (calculateOriginationFee(collateralAmount,originationFee))) * 1 ether;
      }
