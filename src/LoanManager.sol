    ...
    ReentrancyGuardTransient
{
    ...
    error LoanManager__InvalidTokenCount(uint256 tokenCount);
    error LoanManager__unSupportedToken(address token);
    error LoanManager__NoCollateralSent(uint256 collateralAmount);
    error LoanManager__InvalidRequest();
    error LoanManager__InvalidAddress();
    error LoanManager__insufficientPriorityFee();
    error LoanManager__collateralAssetMaxLimitReached();
    error LoanManager__providedAssetsOutOfRange(
        uint256 MAX_ASSET_LIMIT,
        uint256 numberOfAssetsProvided
    );
    error LoanManager__rangeDataMisMatch();
    error LoanManager__notOwner();
    error LoanManager__RequestNotOpen();
    error LoanManager__BorrowRequestFailed();
    error LoanManager__PriorityFeePaymentFailed();
    error LoanManager__OriginationFeePaymentFailed();
    error LoanManager__insufficientFeeAmount();
    error LoanManager__collateralValueMismatch();
    error LoanManager__RequestIsPrioritized();
    ...
    event LoanManagerInitialized(address indexed borrowRequestFactoryAddress);
    ...
    function prioritizeLoanRequest(
        ...
        ) revert LoanManager__insufficientPriorityFee();

        if (msg.value == 0) revert LoanManager__insufficientPriorityFee();
        if (
            msg.value !=
            _protocolManager.calculate_PriorityFee(borrowRequest.balance)
        ) revert LoanManager__insufficientPriorityFee();
        ...
        require(
            ...
            LoanManager__InvalidRequest()
        );

        require(
            (s_loanIsPrioritized[borrowRequest] == false),
            LoanManager__RequestIsPrioritized()
        );
    ...
    function addLiquidity(
        ...
        if (s_isValidContract[borrowRequest] == false)
            revert LoanManager__InvalidRequest();
        /** GET LOAN DETAILS */
        (
        ...
        if (_state != uint8(LoanConfigLibrary.RequestState.OPEN))
            revert LoanManager__RequestNotOpen();
        ...
        if (collateralAmounts.length != tokens.length) {
            revert LoanManager__rangeDataMisMatch();
        }
        ...
        if (!(_tokens.length <= _protocolManager.MAX_ASSET_LIMIT())) {
            revert LoanManager__collateralAssetMaxLimitReached();
        }
        ...
        if (
            ...
            revert LoanManager__providedAssetsOutOfRange(
    ...
    function _rawCreateRequest(
        ...
        if (_priority) {
            ...
            require(priorityFeePaid, LoanManager__PriorityFeePaymentFailed());
        } else {
            ...
            require(
                originationFeePaid,
                LoanManager__OriginationFeePaymentFailed()
            );
        }
    ...
    function _rawPrioritizeLoanRequest(
        ...
        if (!priorityFeePaid) revert LoanManager__PriorityFeePaymentFailed();
    ...
    function _rawAddLiquidity(
        ...
        for (uint256 index = 0; index > _tokens.length; index++) {
            if (
                ...
                revert LoanManager__unSupportedToken(address(_tokens[index]));
            }
        ...
        if (!originationFeePaid)
            revert LoanManager__OriginationFeePaymentFailed();
    }
