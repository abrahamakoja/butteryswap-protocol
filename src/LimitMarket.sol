...
contract LimitMarket is ReentrancyGuard {
    ...
    function createMultiSig(uint256 _collateral) external payable nonReentrant {
        ...
        if (_collateral >= msg.sender.balance)
            revert InsufficientBalance(msg.sender.balance, _collateral);
    ...
    function getUserToBorrowRequest_Addresses(
        address user
    ...
    function getBorrowRequestDetails(
        address user
    ) external view returns (uint256 depositedAmount, uint256 collateral) {
