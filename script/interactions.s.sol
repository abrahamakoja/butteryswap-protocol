...
contract LimitMarketInteractions is Script {
     using erc20TokenLibrary for erc20TokenLibrary.tokenData;
        ...
        address mostrecentlyDeployedEC;
    ...
    function run() external {
        ...
        mostrecentlyDeployedEC = DevOpsTools.get_most_recent_deployment( "Enforcer_v1", block.chainid);
        mostrecentlyDeployedST = DevOpsTools.get_most_recent_deployment("SupportedTokens",block.chainid );

        updateContracts(
            payable(mostrecentlyDeployedLimitMarket),
            mostrecentlyDeployedEC,
    ...
    function updateContracts(
        ...
        console.log("_mostrecentlyDeployedLimitMarket",_mostrecentlyDeployedLimitMarket);
        console.log("enforcer",enforcer);
        LimitMarket_v1(payable(_mostrecentlyDeployedLimitMarket)).updateContracts(enforcer, supportedTokenAddress);
        vm.stopBroadcast();
    }
    ...
    function lend(address _mostrecentlyDeployedLimitMarket) public{
        vm.startBroadcast();
        LimitMarket_v1(payable(_mostrecentlyDeployedLimitMarket)).lend{value: 6 ether}();
        vm.stopBroadcast();
    }
}
